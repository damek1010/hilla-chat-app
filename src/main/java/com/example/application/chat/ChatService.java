...
public class ChatService {
    ...
    private void generateTestData() {
        ...
        for (String channelName : chatChannels) {
            ...
            log.info("Created channel: {} (http://localhost:8080/channel/{})", channel.name(), channel.channelId());
        }
    }
    ...
    public Optional<Channel> channel(ChannelId channelId) {
        return channelRepository.findById(channelId);
    }

    public Flux<List<Message>> liveMessages(ChannelId channelId) throws InvalidChannelException {
    ...
    public List<Message> messageHistory(ChannelId channelId, int fetchMax, @Nullable MessageId lastSeenMessageId) {
        return messageRepository.findLatest(channelId, fetchMax, lastSeenMessageId);
    }

    public void postMessage(ChannelId channelId, String message) throws InvalidChannelException {
        if (!channelRepository.exists(channelId)) {
            throw new InvalidChannelException();
        }
        var author = "John Doe";
