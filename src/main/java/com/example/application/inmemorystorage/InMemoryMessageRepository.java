...
class InMemoryMessageRepository implements MessageRepository {
    ...
    private final ConcurrentMap<ChannelId, MessageArchive> messageArchives = new ConcurrentHashMap<>();

    @Override
    public List<Message> findLatest(ChannelId channelId, int fetchMax, @Nullable MessageId lastSeenMessageId) {
        if (fetchMax < 1) {
    ...
    private static class MessageArchive {
        ...
        private MessageArchive(ChannelId channelId) {
            this.channelId = channelId;
        }

        public List<Message> findLatest(int fetchMax, @Nullable MessageId lastSeenMessageId) {
            lock.readLock().lock();
            try {
        ...
        private int indexOfMessage(MessageId messageId) {
        ...
        public Message save(NewMessage newMessage) {
            lock.writeLock().lock();
            try {
                var message = new Message(new MessageId(UUID.randomUUID().toString()), channelId, sequenceNumber.getAndIncrement(),
