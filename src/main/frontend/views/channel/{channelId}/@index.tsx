...
export default function ChannelView() {
    ...
    async function postMessage(message: string) {
        ...
        try {
            await ChatService.postMessage(channel.value.channelId, message)
        } catch (_) {
    ...
    function subscribe() {
        unsubscribe()
        if (channel.value) {
            console.log("Subscribing to", channel.value.channelId.id)
            subscription.value = ChatService.liveMessages(channel.value.channelId)
                .onNext(receiveMessages)
                .onError(() => console.error("Error in subscription"))
            const lastSeenMessage = messages.value[-1]
            ChatService.messageHistory(channel.value.channelId, HISTORY_SIZE, lastSeenMessage.messageId)
